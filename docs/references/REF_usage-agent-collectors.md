...
3. **聚合事件只作兜底**：WorkBuddy 本机 `traces` 可提供结构化 `toolName=Skill` 调用；只有没有 trace skill 信号时，才回退读取 `usage-log.json` 的 `skills` 聚合日期。
...
| OpenCode | OpenCode 官方 skills 文档说明 skills 通过 native `skill` tool on-demand 加载；本机 SQLite `~/.local/share/opencode/opencode.db` 的 `part.data` 可解析 completed `tool=skill`；当前本机还验证到新式 `/skill` 触发会把 `~/.config/opencode/skills/<skill>/SKILL.md` 正文注入为 user-role `part.type=text`，且 `~/.config/opencode/skills` 实际是符号链接目录；`~/.local/state/opencode/prompt-history.jsonl` 仅保存最近输入、无时间/session/project，不作为主计数来源 | `local session parser`；plugin 可升级为 `direct observed` | `OpenCodeSqliteCollector` 读取 SQLite completed skill parts、user-role 行首 `$skill`/`/skill` 候选、以及与本地 `SKILL.md` 正文匹配的 skill-content injection；插件 collector 作为后续增强 | SQLite 属于本地会话状态，只读取字段白名单；skill 文件只用于正文指纹匹配；不读取普通 read/edit/tool output；prompt-history 不满足时间/项目统计要求 | P1 |
...
| WorkBuddy | 本机 `~/.workbuddy/traces/**/trace_*.json` 含 `spans[]`，结构化 Skill 调用为 `name/toolName=Skill`、`type=function`、`status`、`startedAt`、`toolInput.skill`；`~/.workbuddy/app/sessions.json` 可把部分 `trace.sessionId` 映射到 `workDir`；`~/.workbuddy/usage-log.json` 含 `skills`、`activeDays` 聚合字段 | `direct observed` | `WorkBuddyUsageLogCollector` 保持旧 parser revision 用于替换历史派生数据，但读取策略为 traces 优先、sessions 补项目、usage-log 兜底；不从 generation prompt/response/tool output 推断 | trace 大文件含 prompt/response/tool output，collector 只抽 span 白名单字段；`tasks` 是任务清单不计数；`mcps` 不计数；不套用 CodeBuddy schema | P2 |
