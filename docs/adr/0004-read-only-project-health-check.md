...
Project Doctor assesses one project's managed Skill deployments and external Skills in known Agent skill directories. The CLI is the explicit user-facing project entry; the desktop retains only its existing global Doctor maintenance call and does not expose a project report. CLI accepts an explicit project path, including an unregistered directory, without registering it. Both use the same diagnostic semantics. Omitting a CLI project path retains existing global Doctor behavior.
