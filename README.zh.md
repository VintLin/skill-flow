...
`skill-manager` 用工作流视角重新组织技能管理：从 Git 源添加技能组，选择投影目标，统一更新，诊断漂移。让技能管理清晰、可控、高效。
...
**状态管理**
- `~/.skillmanager/manifest.json` - 你的配置意图
- `~/.skillmanager/lock.json` - 实际部署状态
- `~/.skillmanager/source/git/<source-id>/` - Git 源缓存
...
可通过环境变量自定义目标路径（如 `SKILL_MANAGER_TARGET_CLAUDE_CODE`）。
