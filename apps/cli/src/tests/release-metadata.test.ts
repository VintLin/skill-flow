...
describe("release metadata", () => {
  test("workspace packages, changelog, and release notes are aligned to v1.5.7", async () => {
    ...
    expect(new Set(packageVersions)).toEqual(new Set(["1.5.7"]));
    ...
    const releaseNotes = await fs.readFile(
      path.join(repoRoot, "releases", "RELEASE_v1.5.7.md"),
      "utf8",
    );
    expect(releaseNotes).toContain("# RELEASE v1.5.7");
