...
describe("release metadata", () => {
  test("workspace packages, changelog, and release notes are aligned to v1.4.1", async () => {
    ...
    expect(new Set(packageVersions)).toEqual(new Set(["1.4.1"]));
    ...
    const releaseNotes = await fs.readFile(
      path.join(repoRoot, "releases", "RELEASE_v1.4.1.md"),
      "utf8",
    );
    expect(releaseNotes).toContain("# RELEASE v1.4.1");
    expect(releaseNotes).toContain("Compared with `v1.4.0`");
  });
});
