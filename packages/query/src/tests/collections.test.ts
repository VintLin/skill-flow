...
describe.sequential("collections", () => {
  const sandbox = useSkillFlowSandbox();

  test("creates a collection from leafs across source groups without creating a checkout", async () => {
    ...
    const result = await app.createCollection({
    ...
    expect(await pathExists(path.join(app.store.rootPath, "source", "virtual", "writing-stack"))).toBe(false);
    expect(await pathExists(app.store.getSourceCheckoutPath("collection", "writing-stack"))).toBe(true);
    ...
    expect(deployments).toEqual(
      deployments.map(() => expect.not.objectContaining({ mode: expect.anything() })),
    );
    ...
    const collections = await v2(app).readCollections();
    expect(collections.collections["writing-stack"]).toMatchObject({
      id: "writing-stack",
      displayName: "Writing Stack",
    ...
    expect(collections.collections["writing-stack"]?.members.map((member) => member.origin)).toEqual([
  ...
  test("keeps collection selected leafs after inspect normalizes bindings", async () => {
    const writingRepo = await createRepo(sandbox.sandboxRoot, {
      "skills/drafting/SKILL.md": skillDoc("drafting", "Draft writing."),
    });
    const editingRepo = await createRepo(sandbox.sandboxRoot, {
    ...
    expect((await v2View(app)).lockFile.sources["writing-stack"]?.leafIds)
      .toEqual(memberLeafIds);
  });

  test("treats collection as a source-like group in summaries inspect and preview planning", async () => {
    const writingRepo = await createRepo(sandbox.sandboxRoot, {
      "skills/drafting/SKILL.md": skillDoc("drafting", "Draft writing."),
    });
    const editingRepo = await createRepo(sandbox.sandboxRoot, {
  ...
  test("bootstraps collection leafs and display name from collection state when manifest is stale", async () => {
    const writingRepo = await createRepo(sandbox.sandboxRoot, {
      "skills/drafting/SKILL.md": skillDoc("drafting", "Draft writing."),
    });
    const editingRepo = await createRepo(sandbox.sandboxRoot, {
    ...
    source.displayName = "collection:writing-stack";
  ...
  test("keeps disabled collection skills visible in summaries and inspect", async () => {
    const writingRepo = await createRepo(sandbox.sandboxRoot, {
      "skills/drafting/SKILL.md": skillDoc("drafting", "Draft writing."),
    });
    const editingRepo = await createRepo(sandbox.sandboxRoot, {
  ...
  test("finds collection project scoped deployments during inspect", async () => {
    const writingRepo = await createRepo(sandbox.sandboxRoot, {
      "skills/drafting/SKILL.md": skillDoc("drafting", "Draft writing."),
    });
    const editingRepo = await createRepo(sandbox.sandboxRoot, {
  ...
  test("repairState keeps collection managed deployments", async () => {
    const writingRepo = await createRepo(sandbox.sandboxRoot, {
      "skills/drafting/SKILL.md": skillDoc("drafting", "Draft writing."),
    });
    const editingRepo = await createRepo(sandbox.sandboxRoot, {
    ...
    const beforeRepair = await v2(app).readState();
    expect(
      beforeRepair.lockFile.projections.filter(
        (projection) => projection.status === "active" && projection.sourceId === "writing-stack",
      ),
    ).toHaveLength(2);
    ...
    const afterRepair = await v2(app).readState();
    expect(
      afterRepair.lockFile.projections
        .filter((projection) => projection.status === "active" && projection.sourceId === "writing-stack")
        .map((projection) => projection.leafId)
        .sort(),
  ...
  test("merge groups hides source groups, clears bindings, and stores restore snapshots", async () => {
    ...
    const collections = await v2(app).readCollections();
    expect(collections.collections["writing-stack"]?.hiddenSourceIds).toEqual(["alpha", "beta"]);
    expect(collections.collections["writing-stack"]?.restoreSelections).toEqual({
      alpha: {
        sourceId: "alpha",
        selectedLeafIds: [alphaLeafId],
        enabledTargets: ["codex"],
  ...
  test("restore merged groups re-shows source groups and deletes the collection", async () => {
    const alphaRepo = await createRepo(sandbox.sandboxRoot, {
      "skills/alpha/SKILL.md": skillDoc("alpha", "Alpha writing."),
    });
    const betaRepo = await createRepo(sandbox.sandboxRoot, {
    ...
    const collections = await v2(app).readCollections();
    expect(collections.collections["writing-stack"]).toBeUndefined();
  });

  test("collection creation blocks duplicate projected skill names", async () => {
    const alphaRepo = await createRepo(sandbox.sandboxRoot, {
    ...
    expect(result.errors[0]?.code).toBe("COLLECTION_SKILL_NAME_CONFLICT");
  ...
  test("rejects empty collection name", async () => {
    ...
    expect(result.ok).toBe(false);
    expect(result.errors[0]?.code).toBe("COLLECTION_NAME_EMPTY");
  });

  test("rejects empty collection skills", async () => {
    const app = new SkillFlowApp();
    ...
    expect(result.ok).toBe(false);
    expect(result.errors[0]?.code).toBe("COLLECTION_SKILLS_EMPTY");
  });
});
