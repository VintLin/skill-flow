...
export const USAGE_AGENT_POLICIES = {
  ...
  cursor: {
    ...
    sourceCandidates: ["Cursor Enterprise OTel logs"],
    acceptedSignals: ["OTel log/event 明确包含 skill event 或 skill 名称"],
    rejectedSignals: [...COMMON_REJECTED_SIGNALS, "团队 analytics 汇总", "本地 .cursor 配置或 plans"],
    privacyBoundary: DIRECT_EVENT_PRIVACY,
    planNote: "缺少本机/配置字段样本前保持 parser_unsupported。",
  },
  "grok-build": {
    agent: "grok-build",
    status: "candidate",
    evidenceLevels: ["direct observed", "local session parser"],
    sourceCandidates: ["Grok Build OTel skill_activated event", "~/.grok/sessions/**/*.jsonl", "Grok hooks"],
    acceptedSignals: ["grok_code.skill_activated 且能取得 skill.name", "hook/session 明确提供 skill activation/name"],
    rejectedSignals: [...COMMON_REJECTED_SIGNALS, "只有 tool.usage 但无 skill.name"],
    privacyBoundary: DIRECT_EVENT_PRIVACY,
    planNote: "需要 details gate 或本地 session 字段样本后才能实现命名统计。",
  },
  pi: {
    agent: "pi",
    status: "implemented",
  ...
  workbuddy: {
    ...
    sourceCandidates: ["未找到稳定 WorkBuddy skill/session/telemetry schema"],
    acceptedSignals: ["无可进入主计数的默认信号"],
    rejectedSignals: [...COMMON_REJECTED_SIGNALS, "把 CodeBuddy 文档套用于 WorkBuddy"],
    privacyBoundary: UNSUPPORTED_PRIVACY,
    planNote: "无一手字段证据，保持 parser_unsupported。",
  },
  codebuddy: {
    agent: "codebuddy",
    status: "candidate",
